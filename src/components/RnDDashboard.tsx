...
export default function RnDDashboard({
  ...
  useEffect(() => {
    const refreshImported = (event: Event) => {
      const detail = (event as CustomEvent<{jobId?: string}>).detail;
      if (detail?.jobId === selectedJobId) void loadWorkspace(selectedJobId);
    };
    window.addEventListener("datanest:external-ai-imported", refreshImported);
    return () => window.removeEventListener("datanest:external-ai-imported", refreshImported);
  }, [selectedJobId, loadWorkspace]);
    ...
    {loading ? <div className="loadingBar"><span /></div> : selectedJob && <>
      ...
      <section className="rndPrimaryGrid">
        ...
        <div className="panel rndChatPanel">
          ...
          <div className="rndExternalAiShortcut">
            <div>
              <p className="eyebrow">EXTERNAL AI</p>
              <b>Live external AI now runs in the persistent DataNest sidebar.</b>
              <small>The selected Job Manifest is synchronized automatically.</small>
            </div>
            <button
              className="secondaryButton compact"
              type="button"
              onClick={() => window.dispatchEvent(new Event("datanest:open-ai-sidebar"))}
            >Open AI Sidebar</button>
          </div>
        </div>
