...
-- Staging-only learning evidence and promotion lifecycle.
alter table public.ai_intake_events
  drop constraint if exists ai_intake_events_source_type_check;
alter table public.ai_intake_events
  add constraint ai_intake_events_source_type_check check (
    source_type = any(array[
      'human'::text,'ai_companion'::text,'datanest_ai'::text,'legacy_import'::text,
      'file_upload'::text,'document_evidence'::text,'application'::text
    ])
  );

alter table public.ai_learning_candidates
  add column if not exists promotion_state text not null default 'not_ready',
  add column if not exists promoted_memory_id uuid,
  add column if not exists promoted_at timestamptz,
  add column if not exists promotion_error text;

alter table public.ai_learning_candidates
  drop constraint if exists ai_learning_candidates_promotion_state_check;
alter table public.ai_learning_candidates
  add constraint ai_learning_candidates_promotion_state_check check (
    promotion_state = any(array['not_ready'::text,'ready'::text,'promoting'::text,'promoted'::text,'failed'::text])
  );

alter table public.ai_learning_candidates
  drop constraint if exists ai_learning_candidates_promoted_requires_memory_check;
alter table public.ai_learning_candidates
  add constraint ai_learning_candidates_promoted_requires_memory_check check (
    (promotion_state='promoted' and promoted_memory_id is not null and promoted_at is not null and promotion_error is null)
    or (promotion_state<>'promoted' and promoted_memory_id is null and promoted_at is null)
  );

create index if not exists ai_learning_candidates_promotion_state_idx
  on public.ai_learning_candidates(project_id,promotion_state,updated_at desc);
